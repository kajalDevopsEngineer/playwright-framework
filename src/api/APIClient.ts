...
export class APIClient {
  ...
  async getEmployeeById(id: number): Promise<any> {
    ...
    if (!response.ok()) {
      throw new Error(
        `Get employee ${id} failed: ${response.status()}`
      );
    }
 ...
 * Returns the created employee's data including their ID
 */
async createEmployee(
  ...
  if (!response.ok()) {
    ...
    throw new Error(
      `Create employee failed: ${response.status()}`
    );
  }
...
/**
 * Delete an employee by their empNumber
